...
export default function Login() {
  ...
  const handleLoginSubmit = (e) => {
    ...
    setFormMessage("Validando credenciales locales...");
    setIsLoading(true);

    setTimeout(() => {
      localStorage.setItem('token', 'LOCAL_MOCK_TOKEN');
      localStorage.setItem('user', JSON.stringify({ name: "Ámbar Gaspar", username: identificador }));
      
      setFormMessage(`Bienvenida, Redirigiendo...`);
      setIsSuccess(true);
  ...
  const handleRegisterSubmit = (e) => {
    ...
    setFormMessage("Simulando envío de token dual...");
    setIsLoading(true);
  ...
  const handleVerifyToken = (e) => {
    ...
    if (tokenInput === "123456") {
      alert("Token correcto (Modo Local). Cuenta dada de alta con éxito.");
      setShowTokenModal(false);
      setIsRegistering(false); 
    } else {
      alert("Usa el token de prueba global: 123456");
    }
  };
