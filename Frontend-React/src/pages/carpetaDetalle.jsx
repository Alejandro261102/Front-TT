...
// SIMULACIÓN DE DATOS LOCALES EXTENDIDA (Con las propiedades requeridas para ciberseguridad)
const fetchFolderDetails = async (folderId) => {
  return {
    id: folderId,
    name: folderId === 'fld-2' ? 'Proyecto terminal' : 'Documentos privados',
    createdAt: '15 Octubre 2025', // aquí se podría formatear una fecha real
    totalSize: '14.5 MB', // tamaño total de los archivos contenidos
    security: folderId === 'fld-2' ? 'public' : 'encrypted',
    contents: [
      { id: 'sub-1', type: 'folder', name: 'Borradores de Reporte', componentIcon: <FaFolderOpen />, size: '3 archivos', sizeInBytes: 3, security: 'public', remitente: 'Héctor Alejandro', fecha: 'Hace 2 días', expiracion: 'Permanente', asunto: 'Esquemas preliminares de diagramas UML', acceso: 'Descarga' },
      { id: 'f-1', type: 'file', name: 'Reporte_Avance.pdf', componentIcon: <IoDocumentText />, size: '2.4 MB', sizeInBytes: 2516582, security: 'Cifrado', remitente: 'Ámbar Gaspar (Tú)', fecha: 'Hoy, 10:30 AM', expiracion: '31/Dic/2026', asunto: 'Avance del capítulo de criptografía asimétrica', acceso: 'Solo Vista' },
      { id: 'f-11', type: 'file', name: 'Nóminas_Servicios.xlsx', componentIcon: <IoBarChart />, size: '0.8 MB', security: 'Bloqueado', remitente: 'Dr. Ariel López Rojas', fecha: 'Ayer', expiracion: '15/Jun/2026', asunto: 'Costos proyectados del API de SMS de Azure', acceso: 'Descarga' }
    ]
  };
}
...
export default function CarpetaDetalle() {
  ...
  // Estados nuevos de control de Interfaz e Inspección Lateral
  const [elementoDetalle, setElementoDetalle] = useState(null)
  const [criterioOrden, setCriterioOrden] = useState('nombre'); // 'nombre' o 'tamano'
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // 🌟 CORREGIDO: Se usa 'err' para que ESLint no marque advertencias
        console.error("Error local al abrir el directorio:", err);
        setError('No se pudo cargar el contenido de la carpeta de resguardo.');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  // 🌟 FUNCIÓN DE ORDENAMIENTO EN VIVO
